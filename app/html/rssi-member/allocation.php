...
-- 3. Schedule: one row per (associate, workday) = latest as of month end
sched_current AS (
...
-- 4. Workdays per member (merged employee + others)
member_workdays AS (
    SELECT
        m.associatenumber,
        COUNT(c.attendance_date) AS workdays,
        MIN(c.attendance_date)   AS start_date,
        MAX(c.attendance_date)   AS end_date
    FROM members m
    JOIN calendar c
...
-- 5. Holiday dates string per member
holiday_dates AS (
    SELECT
        m.associatenumber,
...
-- 6. Punch in/out aggregated ONCE per (user, day) — no status split
punch_agg AS (
    SELECT
        a.user_id,
    ...
    SELECT
        ...
        CASE
            ...
            WHEN b.late_entry_time IS NOT NULL THEN
                CASE
                    WHEN b.punch_in IS NOT NULL AND b.punch_in::time <= b.late_entry_time::time THEN 'Exc.'
...
SELECT
    ...
    CASE
        WHEN f.engagement = 'Member' THEN NULL
        ELSE mw.start_date
...
GROUP BY
    f.associatenumber, f.fullname, f.engagement, f.phone, f.doj,
    mw.workdays, mw.start_date, mw.end_date, hd.holiday_dates
