...
if ($hasFilters) {
    ...
    // Add location filter if selected (MOVED HERE - inside the if($hasFilters) block)
    if (!empty($location)) {
...
if (!empty($studentIds)) {
    echo '<strong>Student IDs:</strong> ' . implode(', ', $studentIds) . '<br>';
}
if (!empty($location)) {
    ...
    $locationNameRow = pg_fetch_assoc($locationNameResult);
    $locationName = $locationNameRow['name'];
    echo '<strong>Location:</strong> ' . $locationName . '<br>';
}
