...
<body>
    ...
    <script>
        // Applicant and Interview Data Fetching Script
        ...
        const subjectKnowledge = document.getElementById("subjectKnowledge");
        const computerKnowledge = document.getElementById("computerKnowledge");
        const demoClass = document.getElementById("demoClass");
        const writtenTest = document.getElementById("writtenTest");
        const experience = document.getElementById("experience");
        const remarks = document.getElementById("remarks");
        const interviewerTable = document.getElementById("interviewer_table").getElementsByTagName('tbody')[0];
        ...
        document.getElementById("verifybutton").addEventListener("click", function (event) {
            ...
            $('#verifybutton').prop('disabled', true).text('Loading...');
            ...
            fetch('payment-api.php', {
                ...
                .then(data => {
                    $('#verifybutton').prop('disabled', false).text('Fetch Applicant Data');
                    if (data.status === 'success') {
                        // Populate the Applicant Data fields
                        ...
                        if (data.data.resumeLink) {
                            ...
                            resumeText.textContent = "No resume uploaded yet";
                        }
                        ...
                        if (data.data.interview_data) {
                            alert("This candidate's interview has already been completed.");

                            // Populate the Interview fields
                            // Get the list of documents from the data
                            const documentsList = data.data.interview_data.documentsList || "";

                            // Split the string into an array (based on the comma separator)
                            const selectedDocuments = documentsList.split(',');

                            // Get the select element
                            const documentsSelect = document.getElementById('documents');

                            // Loop through all options and select the ones that are in the selectedDocuments array
                            for (let option of documentsSelect.options) {
                                if (selectedDocuments.includes(option.value)) {
                                    option.selected = true;
                                }
                            }

                            subjectKnowledge.value = data.data.interview_data.subjectKnowledge || "";
                            computerKnowledge.value = data.data.interview_data.computerKnowledge || "";
                            demoClass.value = data.data.interview_data.demoClass || "";
                            writtenTest.value = data.data.interview_data.writtenTest || "";
                            experience.value = data.data.interview_data.experience || "";
                            remarks.value = data.data.interview_data.remarks || "";


                            // Assuming data.data.interview_data.interviewers contains the interviewer data
                            if (data.data.interview_data && Array.isArray(data.data.interview_data.interviewers) && data.data.interview_data.interviewers.length > 0) {
                                const interviewerTableBody = document.querySelector('#interviewer_table tbody');

                                // Loop through the interviewers and add rows to the table
                                data.data.interview_data.interviewers.forEach(interviewer => {
        ...
        `;
                                ...
                                console.error("No interviewer data found in the response.");
                            }

                        } else {
                            alert("Applicant data fetched successfully! No interview data found.");
                        }
                        ...
                        // Clear interview data fields
                        documents.value = '';
                        subjectKnowledge.value = '';
