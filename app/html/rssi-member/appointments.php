...
$query .= " ORDER BY aa.appointment_date DESC, aa.appointment_time DESC";
...
<head>
    ...
    <title>ABHA Appointments | Health Portal</title>
            ...
            <div>
                ...
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0">
                        <li class="breadcrumb-item"><a href="#">Health Portal</a></li>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h6 class="card-title mb-0">Appointments Overview</h6>
                            ...
                            <?php if (pg_num_rows($result) > 0): ?>
                                <?php while ($row = pg_fetch_assoc($result)): ?>
                                    <tr class="appointment-row">
                                        <td>
                                            <span class="text-muted small">#<?= htmlspecialchars($row['appointment_id']) ?></span>
                                        </td>
                                        <td>
                                            <div class="beneficiary-info">
                                                <div class="beneficiary-name"><?= htmlspecialchars($row['beneficiary_name']) ?></div>
                                                <div class="beneficiary-id">ID: <?= htmlspecialchars($row['beneficiary_id']) ?></div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="date-time">
                                                <div class="date"><?= date('d M Y', strtotime($row['appointment_date'])) ?></div>
                                                <div class="time"><?= date('h:i A', strtotime($row['appointment_time'])) ?></div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge status-badge rounded-pill <?= 'badge-' . htmlspecialchars($row['status'] ?? 'scheduled') ?>">
                                                <?= ucfirst(htmlspecialchars($row['status'] ?? 'scheduled')) ?>
                                            </span>
                                        </td>
                                        <td><?= isset($row['created_by']) ? htmlspecialchars($row['created_by']) : '' ?></td>
                                        <td>
                                            <span class="small"><?= date('d M Y', strtotime($row['appointment_created_at'])) ?></span>
                                        </td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                                data-bs-target="#statusModal<?= $row['appointment_id'] ?>" title="Update Status">
                                                <i class="bi bi-pencil"></i>
                                            </button>
                                            <?php if (!empty($row['workflow'])): ?>
                                                <button type="button" class="btn btn-sm btn-outline-secondary ms-1" data-bs-toggle="modal"
                                                    data-bs-target="#historyModal<?= $row['appointment_id'] ?>" title="View History">
