...
if (isset($_POST['update_exam'])) {
    ...
    if ($update_result) {
        ...
        $class_query = "
            SELECT STRING_AGG(DISTINCT class::text, '+') AS exam_class
            FROM exam_marks_data
            WHERE exam_id = $1
        ";
        $class_result = pg_query_params($con, $class_query, array($exam_id));
        $class_row = pg_fetch_assoc($class_result);

        $exam_class = $class_row['exam_class'];  // contains "4" or "4+5" etc.
...
<body>
            ...
            <form method="POST" id="updateForm" class="mt-4">
                ...
                <div class="mb-3">
                    ...
                    <select id="examiner_viva" name="examiner_viva" class="form-select select2">
                        <?php if (!empty($examData['teacher_id_viva'])): ?>

                            <option value="<?php echo htmlspecialchars($examData['teacher_id_viva']); ?>" selected>
                                <?php echo htmlspecialchars($examData['teacher_id_viva'] . ' - ' . ($examData['fullname_viva'] ?? '')); ?>
                            </option>
                    ...
                    <label>
                        <input class="form-check-input" type="checkbox" name="send_email_viva" value="1" <?php echo isset($_POST['send_email_viva']) ? 'checked' : ''; ?>>
                ...
                <div class="mb-3">
                    ...
                    <select id="examiner_written" name="examiner_written" class="form-select select2">
                        <?php if (!empty($examData['teacher_id_written'])): ?>

                            <option value="<?php echo htmlspecialchars($examData['teacher_id_written']); ?>" selected>
                                <?php echo htmlspecialchars($examData['teacher_id_written'] . ' - ' . ($examData['fullname_written'] ?? '')); ?>
                            </option>
                    ...
                    <label>
                        <input class="form-check-input" type="checkbox" name="send_email_written" value="1" <?php echo isset($_POST['send_email_written']) ? 'checked' : ''; ?>>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#examiner_viva,#examiner_written').select2({
                ajax: {
