...
if ($role == 'Admin') {
    ...
    if ($user_id > 0) {
        ...
        FROM bankdetails
        JOIN rssimyaccount_members ON bankdetails.updated_for = rssimyaccount_members.associatenumber WHERE updated_for='$user_id'");
    } else {
        $result = pg_query($con, "SELECT *
        FROM bankdetails
        JOIN rssimyaccount_members ON bankdetails.updated_for = rssimyaccount_members.associatenumber"); //select query for viewing users.
    }
}
if (!$result) {
