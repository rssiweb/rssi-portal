...
?>
<!-- Add the server-side PHP code for processing the form -->
<?php
// Get the reCAPTCHA token from the form data
$token = $_POST['token'];

// Verify the reCAPTCHA token with the reCAPTCHA API
$url = 'https://www.google.com/recaptcha/api/siteverify';
$data = array(
    'secret' => '6LfJRc0aAAAAAFuZLLd3_7KFmxQ7KPCZmLIiYLDH',
    'response' => $token
);
$options = array(
    'http' => array(
...
// If the reCAPTCHA verification is successful, process the form data
if ($response->success) {
    // Get the form data
    ...
    $scode = $_POST['scode'];
    $exit_interview_by = $_POST['exitinterview'];
...
<body>
    <?php if (@$type_of_admission != null && @$cmdtuples == 0) { ?>
        ...
        <div class="alert alert-danger alert-dismissible" role="alert" style="text-align: -webkit-center;">
            ...
            <span class="blink_me"><i class="fa-solid fa-xmark"></i></span>&nbsp;&nbsp;<span>Error: The record has not been updated.</span>
        </div>
    <?php } else if (@$cmdtuples == 1) { ?>

        <div class="alert alert-success alert-dismissible" role="alert" style="text-align: -webkit-center;">
            <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
            <span class="blink_me"><i class="fa-solid fa-xmark"></i></i></span>&nbsp;&nbsp;<span>The record has been updated succesfully.</span>
        </div>
        <script>
        ...
        <?php if (sizeof($resultArr) > 0) { ?>
            ...
            ?>
                <form name="admission_admin" id="admission_admin" action="admission_admin.php" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        <label for="exitinterview">Exit Interview By</label>
                        <input type="text" class="form-control" id="exitinterview" name="exitinterview" placeholder="Enter Exit Interview" value="<?php echo $associatenumber ?>" readonly>
                    </div>
                    <!-- Add the reCAPTCHA token input -->
    ...
    <!-- Add the form submission JavaScript code -->
    <script>
        ...
        document.getElementById('submitBtn').addEventListener('click', function(event) {
            ...
            grecaptcha.ready(function() {
                grecaptcha.execute('6LfJRc0aAAAAAEhNPCD7ju6si7J4qRUCBSN_8RsL', {
                    ...
                    .then(function(token) {
                        // Set the reCAPTCHA token value in the hidden input field
                        document.getElementById('token').value = token;

                        // Submit the form
                        document.getElementById('admission_admin').submit();
                    });
            });
        });
    </script>
