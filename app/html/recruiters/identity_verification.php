...
if ($resultArr && count($resultArr) > 0) {
    ...
    $recruiter_name = $resultArr[0]['full_name'];
    $recruiter_id = $resultArr[0]['id'];
}
if (!$result) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["form-type"]) && $_POST["form-type"] == "verification") {
    ...
    if (empty($_FILES['supportingDocument']['name'])) {
        ...
        $filename_identifier = "$identifier" . "_" . "$recruiter_id" . "_" . time();
        $parent_identifier = '1LqQxq0V9DW6sRhlJL6ZW93eQHbV3OoUP';
    ...
    if ($cmdtuples == 1) {
        sendEmail("tap_identity_document_submitted", array(
            "application_number" => $recruiter_id,
            "applicant_name" => $recruiter_name,
        ), 'info@rssi.in');
    }
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <?php foreach ($resultArr as $array) { ?>
                                <?php
                                $isFormDisabled = (empty($array["is_verified"]) ||
                                    ($array["is_verified"] === 'f' && !empty($array["is_verified"])))
                                    ? null
                                    : 'disabled';
                                ?>
                                <div class="container">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <input type="text" class="form-control" id="identifierNumber" name="identifierNumber" placeholder="Enter your identifier number" value="<?php echo ($array["aadhar_number"]); ?>" required>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <?php if (!empty($array['aadhar_file_path'])): ?>
                                                                    <div class="photo-box mt-2" id="candidatePhotoContainer" style="width: 400px; height: 250px;">
                                                                        ...
                                                                        preg_match('/\/file\/d\/([a-zA-Z0-9_-]+)\//', $array['aadhar_file_path'], $matches);
                                                                        $file_id = $matches[1];
