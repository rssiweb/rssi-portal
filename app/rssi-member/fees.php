...
@$status = $_POST['get_id'];
  ...
  $result = pg_query($con, "SELECT * FROM fees WHERE month=$status AND DATE_PART('year', date::date)=$id order by id desc");
  $totalapprovedamount = pg_query($con, "SELECT SUM(fees) FROM fees WHERE month=$status AND DATE_PART('year', date::date)=$id");
          ...
          <?php if ($resultArr != null) {
            ...
            foreach ($resultArr as $array) {
              echo '<tr><td>' . substr($array['date'], 0, 10) . '</td>
                        <td>' . $array['studentid'] . '/' . strtok($array['sname'], ' ') . '</td>   
                        <td>' . $array['fees'] . '</td>
